...
create_files()
{
    while [ "${#@}" -gt 0 ] ; do
        ...
        # copy new top level Makefile if present
        if [ "${src}" == "./gMakefile" ]
        then
            create_file ./gMakefile Makefile "Generic Makefile" update
            continue ;
        fi

        # install new installer.bash if present
        if [ "${src}" == "./installer.bash" ]
        then
            # must remove first - shell currently has it open so do not overwrite it
            rm -f "${UPDATES_DIR}/installer.bash"
            cp "${src}" "${UPDATES_DIR}/installer.bash"
            continue ;
        fi

        # copy the Makefile and README to updates/Makefile or updates/README respectively
...
fix_executable()
{
    while [ "${#@}" -gt 0 ] ; do
        ...
        *)
            ...
            # ignore gMakefile, Makefile and README
            if [ "${src}" == "Makefile" -o "${src}" == "gMakefile" -o "${src}" == "README" ] ; then continue ; fi
