...
// ***** WHAT TO DO *****
//
// MODIFY the skeleton code below to make an annotated copy an abstract syntax tree, ast, of a Jack class.
//
...
// check if return flag is true
bool flag_return = false;

// global var for true|false
bool t_or_f = false;
...
ast copy_class(ast t)
{
    ...
    if (var_decs_copy == var_decs && subr_decs_copy == subr_decs)
        return t;
...
ast copy_class_var_decs(ast t)
{
    ...
    {
        // adds the comment annotation for class var decs
        a = get_ann(t);
        a = add_ann_comments(a, "private:");
    }

    // returns annotation
    return create_class_var_decs(a, decs);
}
...
ast copy_subr_decs(ast t)
{
    ...
    {
        // returns the annotation for subr decs
        a = get_ann(t);
        a = add_ann_comments(a, "public:");
    }
    // returns annotation
    return create_subr_decs(a, decs);
}
...
ast copy_constructor(ast t)
{
    ...
    if (param_list_copy == param_list && subr_body_copy == subr_body)
    {
        return t;
    }
    // returns annotation
    ...
    {
        ann a = get_ann(t);
        //a = add_ann_errors(a, "Duplicate declaration");
        return create_constructor(a, vtype, name, param_list_copy, subr_body_copy);
    }
}
...
ast copy_subr_body(ast t)
{
    ...
    if (decs_copy == decs && body_copy == body)
        return t;
...
ast copy_statements(ast t)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        if (ast_have_kind(deci, ast_return))
        {
            flag_return = true;
        }
    ...
    // returns annotation
    return create_statements(a, decs);
}
...
ast copy_statement(ast t)
{
    ...
    // if one of the statement nodes found
    ...
    // if (flag_return == true){
    //     a = add_ann_warnings(a, "Unreachable");
    // }

    //a = add_ann_warnings(a, "Unreachable");

    // returns annotation
    return create_statement(a, copy);
}
...
ast copy_let(ast t)
{
    ...
    if (var_copy == var && expr_copy == expr)
        return t;
...
ast copy_let_array(ast t)
{
    ...
    if (var_copy == var && index_copy == index && expr_copy == expr)
        return t;
...
ast copy_if(ast t)
{
    ...
    if (condition_copy == condition && if_true_copy == if_true)
        return t;
    ...
    // returns annotation
    return create_if(a, condition_copy, if_true_copy);
}
...
ast copy_if_else(ast t)
{
    ...
    if (condition_copy == condition && if_true_copy == if_true && if_false_copy == if_false)
        return t;
    ...
    // returns annotation
    return create_if_else(a, condition_copy, if_true_copy, if_false_copy);
}
...
ast copy_while(ast t)
{
    ...
    // if condition contains a single ast_term node 
    // containing an ast_bool node that has value false
    ...
    if (condition_copy == condition && body_copy == body)
    {
        return t;
    }

    // returns annotation
    return create_while(a, condition_copy, body_copy);
}
...
ast copy_return_expr(ast t)
{
    ...
    ann a = get_ann(t);
    //a = add_ann_comments(a, "what");

    if (copy == expr)
        return t;

    // returns annotation
    return create_return_expr(a, copy);
}
...
ast copy_array_index(ast t)
{
    ...
    if (var_copy == var && index_copy == index)
        return t;
...
ast copy_call_as_method(ast t)
{
    ...
    if (var_copy == var && subr_call_copy == subr_call)
        return t;
